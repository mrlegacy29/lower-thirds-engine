...
| `test/*.js` + `test/run-all.js` | 22 jsdom suites covering every control, clear-detection (F1/F2), media/motion, layering/dock, operator mode, take-state, glow/shape/bevel, source fallback across both render paths, and the SDI surface. | Add a suite when you add a feature. |
...
3. **Always run `npm test` before building or releasing.** `run-all.js`
   syntax-checks `lt.html` and runs all 22 suites. Green = safe to ship.
...
npm test         # syntax-check lt.html + run all 22 suites  (MUST be green)
